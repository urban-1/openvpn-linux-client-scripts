...
function start(){
  ...
  echo "Saving old GateWay"
  echo "$next_hop $orig_dev" > /tmp/ovpn_gw_${conf};
...
# DOESN'T WORK WHEN DNS IS ON DIFFERENT SUBNET
...
#   orig_dev=`route -n | grep -v $iface | grep $remote_ip |  awk -F' +'   '{ print $8;}'`
  next_hop=`cat /tmp/ovpn_gw_${conf} | cut -d' ' -f1`
  orig_dev=`cat /tmp/ovpn_gw_${conf} | cut -d' ' -f2`
